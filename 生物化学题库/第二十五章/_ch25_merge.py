# -*- coding: utf-8 -*-
"""第二十五章 批次合并脚本:组装 questions.json / terms.json,重排 id"""
import sys, json, importlib.util
sys.stdout.reconfigure(encoding='utf-8')

base = r'C:\Users\Lenovo\Desktop\湖南大学\生物化学题库\第二十五章'
...
all_q = []
for b in ['_ch25_batch1', '_ch25_batch2', '_ch25_batch3', '_ch25_batch4', '_ch25_batch5']:
    qs = load(b)
    print(f'{b}: {len(qs)} 题')
    all_q.extend(qs)
...
# terms
spec = importlib.util.spec_from_file_location('_ch25_terms', f'{base}\\_ch25_terms.py')
m = importlib.util.module_from_spec(spec)
spec.loader.exec_module(m)
terms = m.terms
...
for t in terms:
    n = len(t['definition'])
    flag = 'OK' if 30 <= n <= 80 else '<<< 超范围'
    print(f"  #{t['id']} {t['term'][:20]}: {n}字 {flag}")
